...
import { useUpdateConversationMutation } from '~/data-provider';
import EndpointIcon from '~/components/Endpoints/EndpointIcon';
import { useConversations, useNavigateToConvo, useLocalize } from '~/hooks';
import { NotificationSeverity } from '~/common';
import { ArchiveIcon } from '~/components/svg';
import { useToastContext } from '~/Providers';
import DropDownMenu from './DropDownMenu';
...
export default function Conversation({ conversation, retainView, toggleNav, isLatestConvo }) {
  ...
  const genTitleMutation = useGenTitleMutation({
    onSuccess: (data, vars) => {
      if (vars.conversationId) {
        updateConvoMutation.mutate(
          { conversationId: vars.conversationId, title: data.title },
          {
            onSuccess: () => refreshConversations(),
          },
        );
    ...
    onError: () => {
      showToast({
        message: localize('com_ui_gen_title_error'),
        severity: NotificationSeverity.ERROR,
        showIcon: true,
      });
    },
  ...
  const aiRenameHandler = (e: MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    genTitleMutation.mutate({ conversationId });
  };

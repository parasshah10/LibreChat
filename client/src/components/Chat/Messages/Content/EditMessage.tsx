...
const EditMessage = ({
  ...
  const [editedText, setEditedText] = useState<string>(text ?? '');
  const textAreaRef = useRef<HTMLTextAreaElement | null>(null);
  const [isAddingNewMessage, setIsAddingNewMessage] = useState(false); // New state
  ...
  const resubmitMessage = () => {
    if (message.isCreatedByUser) {
      ...
      setIsAddingNewMessage(true); // New message is being added
      setSiblingIdx((siblingIdx ?? 0) - 1);
    } else {
      const messages = getMessages();
      const parentMessage = messages?.find((msg) => msg.messageId === parentMessageId);
      ...
      setIsAddingNewMessage(false); // Editing existing message
      setSiblingIdx((siblingIdx ?? 0) - 1);
    }
